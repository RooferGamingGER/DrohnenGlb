...
export const useModelViewer = ({ containerRef }: UseModelViewerProps) => {
      ...
      if (rendererRef.current && sceneRef.current && cameraRef.current) {
        sceneRef.current.traverse((child) => {
          if (child instanceof THREE.Sprite && child.userData && child.userData.isLabel) {
            ...
            const distance = cameraRef.current!.position.distanceTo(child.position);
  ...
  const loadModel = async (file: File): Promise<THREE.Group> => {
    try {
  ...
  const takeScreenshot = useCallback(() => {
    if (rendererRef.current) {
      ...
      link.href = imgData;
      link.download = 'drohnenaufmass-' + new Date().toISOString().slice(0, 10) + '.png';
