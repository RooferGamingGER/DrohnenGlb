...
// New: Update the area preview display
export const updateAreaPreview = (
  ...
  clearPreviewObjects(scene);
  
  // Create preview line connecting last point to first point
  const points = tempPoints.map(p => p.position.clone());
  const previewLinePoints = [
    points[points.length - 1].clone(),
    points[0].clone()
  ];
  
  const previewLine = createPreviewLine(previewLinePoints);
  scene.add(previewLine);
  
  // Calculate the area
  const area = calculatePolygonArea([...points, points[0].clone()]);
  
  // Create a label for the area
  // Position label at the centroid of the polygon
  const centroid = new THREE.Vector3();
  points.forEach(p => centroid.add(p));
  centroid.divideScalar(points.length);
  
  // Offset the label slightly above the polygon
  const normal = calculatePolygonNormal(points);
  centroid.add(normal.multiplyScalar(0.2));
  
  const areaText = formatArea(area);
  const previewLabel = createPreviewTextSprite(`${areaText} (Vorschau)`, centroid);
  scene.add(previewLabel);
  
  // Update the dynamic scale of the label
  updateLabelScale(previewLabel, camera);
  
  // Store references to preview objects for later cleanup
  measurement.previewLineObject = previewLine;
  measurement.previewLabelObject = previewLabel;
  measurement.value = area; // Update the measurement value
};

// New: Calculate the normal vector of a polygon
export const calculatePolygonNormal = (points: THREE.Vector3[]): THREE.Vector3 => {
  if (points.length < 3) return new THREE.Vector3(0, 1, 0);
  
  // Use the first three points to calculate a normal
...
// New: Clear preview objects from the scene
export const clearPreviewObjects = (scene: THREE.Group): void => {
  // Find and remove objects with isPreviewLine or isPreview flags
  scene.traverse((object) => {
    if (object.userData && (object.userData.isPreviewLine || object.userData.isPreview)) {
      if (object.parent) {
        object.parent.remove(object);
      }
...
// New: Finalize a polygon measurement by removing previews and creating final geometry
export const finalizePolygon = (measurement: Measurement, scene: THREE.Group): void => {
  ...
  // Clear the references in the measurement object
  if (measurement.previewLineObject) {
    if (measurement.previewLineObject.parent) {
    ...
    if (measurement.previewLineObject.material instanceof THREE.Material) {
  ...
  if (measurement.previewLabelObject) {
    if (measurement.previewLabelObject.parent) {
    ...
    if (measurement.previewLabelObject.material instanceof THREE.SpriteMaterial && 
        measurement.previewLabelObject.material.map) {
      measurement.previewLabelObject.material.map.dispose();
    }
    
    if (measurement.previewLabelObject.material instanceof THREE.Material) {
  ...
  if (measurement.points.length >= 3) {
    ...
    if (firstPoint.position.distanceTo(lastPoint.position) > 0.001) {
  ...
  // Update the measurement geometry to reflect the final state
